...
const (
	...
	initializeBody         = `{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","clientInfo":{"name":"header-probe","version":"1.0.0"}}}`
)

// demoContext 保存运行 header 验证 case 所需的上下文。
...
// headerObservation 表示 header 验证结果。
type headerObservation struct {
	DebugRequestHeader  string
	DebugResponseHeader string
	RawBody             string
}

// Run 创建 MCP_REMOTE 工具并验证 Hook 可以改写请求头和响应头。
func Run() error {
	...
	dctx := &demoContext{
		...
		sdkClient:    sdkClient,
		fcClient:     fcClient,
		toolName:     "hook-quickstart-header-" + suffix,
		mcpFuncName:  "hook-qs-header-mcp-" + suffix,
		hookFuncName: "hook-qs-header-hook-" + suffix,
	}

	return dctx.run()
}

// run 部署示例服务、创建工具并执行 header 验证。
func (d *demoContext) run() error {
	slog.Info("开始运行 MCP header hook case", "tool", d.toolName, "dataEndpoint", d.dataEndpoint)

	if err := buildBinary("headerdesk", filepath.Join(d.moduleDir, "services", "headerdesk"), filepath.Join(d.binDir, "headerdesk")); err != nil {
		return err
	}
	...
	mcpURL, err := deployBinaryAsFunction(d.ctx, d.fcClient, d.uid, filepath.Join(d.binDir, "headerdesk"), "headerdesk", d.mcpFuncName)
	if err != nil {
		return err
	}
	slog.Info("远程 header MCP 函数已部署", "url", mcpURL)
	if err := warmupFunction(d.ctx, mcpURL, 2*time.Minute); err != nil {
		return fmt.Errorf("预热远程 header MCP 失败: %w", err)
	}
	...
	slog.Info("Header Hook 函数已部署", "url", hookURL)
	...
	obs, mcpEndpoint, err := d.verifyHeaderEffects(3)
	if err != nil {
		return err
	}

	slog.Info("MCP header hook case 通过", "tool", d.toolName, "toolId", toolID)
	fmt.Printf("tool=%s\n", d.toolName)
	fmt.Printf("tool_id=%s\n", toolID)
	fmt.Printf("remote_mcp=%s\n", remoteURL)
	fmt.Printf("hook=%s\n", hookEndpoint)
	fmt.Printf("data_plane=%s\n", mcpEndpoint)
	fmt.Printf("debug_request_header=%s\n", obs.DebugRequestHeader)
	fmt.Printf("debug_response_header=%s\n", obs.DebugResponseHeader)
	return nil
}

// assertRemoteFunctions 校验工具已开启 proxy 且保存了两个 header hook。
// 参数 d 是当前运行上下文。
func (d *demoContext) assertRemoteFunctions() (string, error) {
	...
	if !storedProxy || storedHooks != 2 {
		return "", fmt.Errorf("后端存储的 hook 配置不符合预期: proxyEnabled=%v hooks=%d", storedProxy, storedHooks)
	}

	proxyFunc := "agentrun-proxy-" + toolID
	if err := waitForFunction(d.fcClient, proxyFunc, time.Minute); err != nil {
		slog.Warn("等待 proxy 函数失败，继续用数据面验证", "function", proxyFunc, "error", err)
	}
	return toolID, nil
}
