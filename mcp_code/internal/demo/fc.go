...
// deployFunction 创建或更新 FC Hook 函数。
// 参数 ctx 是调用上下文；client 是 FC 客户端；funcName 是函数名；codePackage 是 OSS 代码包地址。
func deployFunction(ctx context.Context, client *fc.Client, funcName string, codePackage codePackageLocation) error {
	if err := codePackage.validate(); err != nil {
		return err
	}
	code := &fc.InputCodeLocation{}
	code.SetOssBucketName(codePackage.OSSBucketName)
	code.SetOssObjectName(codePackage.OSSObjectName)
...
// ensureHTTPTrigger 确保 Hook 函数存在匿名 HTTP 触发器。
// 参数 client 是 FC 客户端；funcName 是函数名。
...
// 参数 client 是 FC 客户端；funcName 是函数名；timeout 是等待超时时间。
